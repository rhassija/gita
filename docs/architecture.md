...
Since Gemini embeddings (3072 dimensions) and Ollama embeddings (768 dimensions) are incompatible in the same database collection, the database manager dynamically routing connection requests:
* **Google Gemini Active**: Resolves database directory to `./chroma_db_backup`.
* **Local Ollama Active**: Resolves database directory to `./chroma_db_ollama`.
* If a provider swap occurs, the backend instantiates and caches a new `chromadb.PersistentClient` for the target path dynamically. No server restarts are required.
